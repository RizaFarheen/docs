...
description: "Learn how to use an agentic research workflow template that generates literature reviews or research gap reports using LLMs, search grounding, and report generation."
---
...
The agentic research template is only available in the [Developer Edition](https://developer.orkescloud.com/) on Launch Pad. 

**To import the template:**

1. Go to [Developer Edition](https://developer.orkescloud.com/) > **Launch Pad** on the left navigation menu.
2. In **Pre-built Workflow Templates**, select **Agentic Research**, and then choose **Use Template**. 
3. Select **Import**.  
...
Otherwise, the workflow will continue to a [Switch](/reference-docs/operators/switch) task, which routes to the relevant [Set Variable](/reference-docs/operators/set-variable) task that prepares the exact configuration of the task(s) to be carried out later. This routing step is in preparation for the [Dynamic Fork](/reference-docs/operators/dynamic-fork) task later on, which requires the task configuration as input.
...
Finally, once the research is completed, another [LLM Chat Complete](/reference-docs/ai-tasks/llm-chat-complete) task is used to synthesize the sub-topics’ findings into a formatted report that answers the user’s original question. A custom [Worker task](/reference-docs/worker-task) is used to generate a .pdf file that downloads directly to the user’s desktop for convenient access.
