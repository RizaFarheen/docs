...
description: "Learn how to use a claims processing workflow template that validates policy eligibility, creates claims, and routes human review for insurance assessment and investigation."
---

# Claims Workflow

:::note  
This template is available only in the [Developer Edition](https://developer.orkescloud.com/) on Launch Pad.   
:::
...
This template provides a quick start for building business process automation workflows that incorporate human inputs, reviews, or approvals.
...
The claims workflow template is available only in the [Developer Edition](https://developer.orkescloud.com/) on Launch Pad. 
...
The `insurance_claim`  workflow is now imported and ready to use.
...
This section explains the workflow logic and execution flow.
...
* The workflow begins with a [Worker task](/reference-docs/worker-task) that retrieves the claimant’s insurance policies.
* Next, the [Inline task](/reference-docs/system-tasks/inline) extracts the claimant’s policy types (Home, Auto, etc.) as options for the claimant to choose in the next Human task.
* The first [Human task](/reference-docs/operators/human)  is intended for the claimant, who must fill out the claim details, including the incident location, the policy type under which the claim is being made, and a description of the incident.
* The workflow continues to a [Switch task](/reference-docs/operators/switch) that evaluates whether the selected policy type is Auto.  
...
* Next, the second [Human task](/reference-docs/operators/human) is intended for the insurance assessor, who reviews the claimant’s coverage eligibility based on the provided claim details. 
* The [Switch task](/reference-docs/operators/switch) then routes the workflow based on the coverage outcome in the previous task, such as Not Covered, Partially Covered, or Full Coverage. 
...
* If the damages exceed the coverage amount, the workflow proceeds to another [Human task](/reference-docs/operators/human), which the claimant must complete with additional incident details for further investigation.
...
This template is simplified to support Auto policies only. To support additional policy types such as Home or Life, add corresponding cases to the Switch task that evaluates the policy type.
...
The workflow contains tasks for retrieving a policy based on the customer’s details and for creating a claim. The final .pdf report is created using a custom Worker task that generates a file from raw report content and a supplied file name.

To utilize these tasks, you must set up workers locally and connect them to the Conductor server using access credentials.
...
With the workflow fully set up, run it.

:::tip  
Refer to the test data for the workflow in the [GitHub README](https://github.com/conductor-oss/awesome-conductor-apps/tree/main/typescript/claims-workflow#test-data-pre-filled).  
:::
...
:::note  
For development purposes, you can complete the Human tasks from the Conductor UI. In production, you can switch the Human tasks to be displayed on your own UI, such as a claims portal or an internal business site, for the relevant users to complete. [Learn more](https://orkes.io/content/developer-guides/orchestrating-human-tasks#step-4-optional-display-the-form-on-your-own-ui).
:::
...
## Workflow output
 ...
 The claims workflow can terminate at different points based on claim eligibility:
...
This template serves as a starting point for customizing the workflow to meet your specific needs. You can modify the workflow by:
