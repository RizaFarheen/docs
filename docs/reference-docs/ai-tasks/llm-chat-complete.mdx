...
| inputParameters. **stopWords** | A list of strings that, when produced by the model, cause generation to stop immediately. These are stop sequences — the model halts output as soon as it produces a matching token — not words filtered from the output. Accepts string and object/array. | Optional. |
