...
| Schema Registry API Secret | The [schema registry API secret](/content/integrations/message-broker/confluent-kafka#get-the-schema-registry-server) obtained from the schema registry server. |  Required if<ul><li>**_Sending Protocol_** is **AVRO**.</li><li>**_Schema Registry Auth Type_** is **Schema Registry User Info (Key/Password)**.</li></ul> | 
