...
description: "Learn how to create and run workers that poll for tasks and execute custom business logic in workflows."
---
...
* **Statelessness**: Workers are stateless and do not implement workflow-specific logic, such as deciding which task comes next.
...
All workers must have a task definition name, which will be used for reference during task polling and execution. The task definition name may be different from the name of the function or method that the worker is responsible for.
...
3. Enter the task details, such as the rate limits, retry settings, timeout settings, and expected inputs and outputs. <br/> The **Name** must match the task name defined previously in your code.
...
1. Configure an application account.
    ...
    2. Create a new application or select an application to which you will be adding your worker. Ensure that the application role has **Worker** enabled.
...
2. Get the application access key for your worker project.
    1. In **Access Keys**, select **Create access key** and store your credentials securely.
    2. Set the Key ID and Key Secret in your project.
3. Grant Execute permission to the application.
    1. In **Permissions**, select **+ Add permission**.
...
To use a [Worker task](/reference-docs/worker-task), add it to a workflow definition.
...
[Run the workflow](https://orkes.io/content/developer-guides/running-workflows) to ensure that your worker has started successfully. If not, return to the previous steps and verify that all details have been entered correctly, such as:
...
* [Guide to Scaling Workers](https://orkes.io/content/developer-guides/scaling-workers) 
* [Implement domains for your workers to split the traffic](https://orkes.io/content/developer-guides/task-to-domain)
