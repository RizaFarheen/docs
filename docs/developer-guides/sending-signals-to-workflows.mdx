...
You can manage and automate workflows in Orkes Conductor by sending signals to pause, resume, terminate, restart, rerun, and more. Signals can be sent in different ways:

- **Using Conductor UI**: Perform actions such as pause, resume, restart, rerun, retry, or terminate directly from the workflow execution page.
- **Using APIs**: Send REST API requests to control workflows programmatically.
- **Using SDKs**: Integrate workflow control into your applications using SDKs in Java, Go, Python, C#, JavaScript/TypeScript, or Clojure.

## Run workflow

You can run workflows programmatically using one of the following endpoints:
* [Start Workflow Execution (Async)](/reference-docs/api/workflow/start-workflow-execution): Start a workflow without waiting for it to be completed.
* [Execute Workflow Synchronously](/reference-docs/api/synchronous-workflow-execution): Start a workflow and wait for an output from the workflow.
...
<p align="center"><img src="/content/img/pause-workflow.png" alt="Pausing workflows from UI" width="100%" height="auto"></img></p>
...
```python
WorkflowResourceApi.pause_workflow(self, workflow_id, **kwargs)
```
...
<p align="center"><img src="/content/img/resume-workflow.png" alt="Resuming workflows from UI" width="100%" height="auto"></img></p>
...
```python
WorkflowResourceApi.resume_workflow(self, workflow_id, **kwargs)
```
...
<p align="center"><img src="/content/img/restart-workflow.png" alt="Restarting workflows from UI" width="100%" height="auto"></img></p>
...
```python
WorkflowResourceApi.restart(self, workflow_id, **kwargs)
```
...
<p align="center"><img src="/content/img/rerun-from-task.png" alt="Rerun workflow from task in Conductor UI" width="100%" height="auto"></img></p>
...
```python
WorkflowResourceApi.retry(self, workflow_id, **kwargs)
```
...
<p align="center"><img src="/content/img/update-task.gif" alt="Update a task's status in Conductor UI" width="90%" height="auto"></img></p>
...
```python
WorkflowResourceApi.terminate(self, workflow_id, **kwargs)
```
