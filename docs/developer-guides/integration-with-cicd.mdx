...
Use the Conductor UI to download each JSON individually, or the API to download the JSONs in bulk.
...
To use the script, you must inject `CONDUCTOR_AUTH_KEY`, `CONDUCTOR_AUTH_KEY`, and `CONDUCTOR_AUTH_SECRET` (ending in /api) variables with the appropriate values in your deployment pipeline. Refer to [Authentication](/sdks/authentication) for a guide on retrieving your Orkes access tokens.
